<x-app-layout>
    ...
    <div class="py-6">
        <div class="mx-auto max-w-7xl sm:px-6 lg:px-8">
            ...
            <div class="overflow-hidden bg-white shadow-sm transition-all duration-300 hover:shadow-md dark:bg-gray-700 sm:rounded-t-lg">
                ...
                <div class="flex items-center justify-between border-b border-gray-200 bg-white p-4 dark:bg-gray-700">
                    <div class="flex items-center space-x-4">
                        ...
                        <div x-data="{ isOpen: false }" class="relative flex justify-center">
                            <button @click="isOpen = true" id="newPresacionBtn"
                            ...
                            <div x-show="isOpen" x-transition:enter="transition duration-300 ease-out"
                                ...
                                <div
                                    ...
                                    <form method="POST" action="{{ route('rh.documentos.store') }}">
                                        ...
                                        <div class="mt-4">
                                            <label class="text-sm text-gray-700 dark:text-gray-200" for="vigencia">Vigencia</label>
                                            <div class="-mx-1 mt-2 flex items-center">
                                                <input type="text" value="{{ old('vigencia') }}" id="vigencia" name="vigencia"
                                                    class="mx-1 block h-10 flex-1 rounded-md border border-gray-200 bg-white px-4 text-sm text-gray-700 dark:border-gray-700 dark:bg-gray-900 dark:text-gray-300 dark:focus:border-blue-300">
                                            </div>
                                        </div>
                        ...
                        <span class="rounded-full bg-white px-3 py-1 text-sm text-gray-600 shadow-sm dark:bg-gray-700 dark:text-gray-300">
                            {{ count($documentos) }} Documentos
                        </span>
                    </div>
                    <div class="relative">
                        <input type="text" id="quickSearch" placeholder="Buscar documento..."
                ...
                {{-- Tabla --}}
                <div x-data="{ isModalOpen: false, id: '', nombre: '', vigencia: '' }" class="overflow-x-auto">
                    <table class="min-w-full divide-y divide-gray-200" id="documentosTable">
                        <thead class="bg-[#D3D8DB] dark:bg-gray-800">
                            <tr>
                                ...
                                <th class="sortable cursor-pointer px-6 py-4 text-left text-xs font-medium uppercase tracking-wider text-gray-500 hover:text-gray-900 dark:text-gray-300"
                                    scope="col" data-column="nombrre">
                                    Vigencia <i class="fas fa-sort ml-1"></i>
                                </th>
                        ...
                        <tbody class="divide-y divide-[#A0B4CB] bg-white dark:bg-gray-700" id="documentoTableBody">
                                ...
                                <tr class="hover:bg-gray-2 00 transform transition-all hover:scale-[1.002] dark:hover:bg-gray-600">
                                    ...
                                    <td class="whitespace-nowrap px-6 py-4 text-sm font-medium">
                                        <div class="flex justify-center space-x-2">
                                            <button
                                                @click="isModalOpen = true; id = '{{ $documento->id }}'; nombre = '{{ $documento->nombre }}'; vigencia = '{{ $documento->vigencia }}'"
                                                id="newPresacionBtn" class="rounded-full p-2 text-yellow-600 transition-colors hover:bg-yellow-50 hover:text-yellow-900">
                                                <i class="fas fa-edit"></i>
                                            </button>
                                            <button class="rounded-full p-2 text-red-600 transition-colors hover:bg-red-50 hover:text-red-900" title="Eliminar"
                                                data-tooltip-target="tooltip-delete" onclick="confirmDelete({{ $documento->id }})">
                        ...
                        <div x-show="isModalOpen" x-transition:enter="transition duration-300 ease-out"
                            ...
                            <div
                                ...
                                <div class="mt-4">
                                    <label class="text-sm text-gray-700 dark:text-gray-200" for="vigencia">Vigencia</label>
                                    <div class="-mx-1 mt-2 flex items-center">
                                        <input type="text" x-model="vigencia" id="editVigencia" name="vigencia"
                                            class="mx-1 block h-10 flex-1 rounded-md border border-gray-200 bg-white px-4 text-sm text-gray-700 dark:border-gray-700 dark:bg-gray-900 dark:text-gray-300 dark:focus:border-blue-300">
                                    </div>
                                </div>
                                <div class="mt-4 sm:-mx-2 sm:mt-6 sm:flex sm:items-center">
                                    ...
                                    <button type="button" onclick="editdocumento()"
                                        class="mt-3 w-full transform rounded-md bg-indigo-600 px-4 py-2 text-sm font-medium capitalize tracking-wide text-white transition-colors duration-300 hover:bg-indigo-700 focus:outline-none sm:mx-2 sm:mt-0 sm:w-1/2">
                                        Guardar
                                    </button>
                                </div>
                    ...
                    <div class="flex flex-1 items-center justify-between">
                        <div class="text-sm text-gray-700 dark:text-gray-300">
                            @if ($documentos->count() > 0)
                                Mostrando <span class="font-medium">{{ $documentos->firstItem() }}</span>
                                a
                                <span class="font-medium">{{ $documentos->lastItem() }}</span> de
                                <span class="font-medium">{{ $documentos->total() }}</span> resultados
                            @else
                        ...
                        <div class="flex space-x-2">
                            ...
                            @else
                                <a href="{{ $documentos->previousPageUrl() }}"
    ...
    @push('scripts')
        <script>
            ...
            function applyFilters() {
                ...
                const rows = document.querySelectorAll('#documentoTableBody tr');
            ...
            document.getElementById('newPresacionBtn').addEventListener('mouseenter', function() {
                this.classList.add('animate-bounce');
            });

            document.getElementById('newPresacionBtn').addEventListener('mouseleave', function() {
    ...
    <script>
        ...
        function confirmDelete(id) {
            if (confirm('¿Estás seguro de que deseas eliminar esta documento?')) {
                axios.post('{{ route('rh.documentos.destroy', ':id') }}'.replace(':id', id), {
        ...
        function editdocumento() {
            ...
            let vigencia = document.getElementById('editVigencia').value;
            const formData = new FormData();
            formData.append('_method', 'PUT')
            formData.append('nombre', nombre);
            formData.append('vigencia', vigencia);
            console.log(id);
            axios.post(`/rh/documentos/${id}`, formData, {
                headers: {
