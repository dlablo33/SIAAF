                ...
                <div class="z-10 flex items-center justify-between border-b border-gray-200 bg-white p-4 dark:bg-gray-700">
                    <div class="flex items-center space-x-4">
                        ...
                        @php
                            ...
                            $now = now();
                            $currentWeek = $now->subWeek()->weekOfYear; // Semana pasada
                        ...
                        <div class="relative w-64">
                            <select name="periodo" id="periodo"
                                ...
                                @for ($week = $currentWeek; $week >= 1; $week--)
                                    @php
                                        $startDate = $now->copy()->setISODate($currentYear, $week)->startOfWeek();
                                        $endDate = $startDate->copy()->endOfWeek();
                                        $periodValue = $week + 1 > 0 ? $week + 1 : 1; // Movemos el numero del periodo para que coincida con el que se una
                                        $displayWeek = $week; // Mantenemos la semana real para las fechas
                                        $isSelected = $week == $currentWeek;
                                    @endphp
                                    <option value="{{ $periodValue }}" {{ $isSelected ? 'selected' : '' }}>
                                        Periodo {{ $periodValue }} ({{ $startDate->format('d/m') }} - {{ $endDate->format('d/m') }})
                                    </option>
                                @endfor
                            </select>
                        </div>
                                ...
                                <tr>
                                    ...
                                    <td class="whitespace-nowrap px-6 py-3 text-center text-sm font-bold text-gray-900 dark:text-white">
                                        <div class="flex flex-col space-y-3">
                                            <div>
                                                <span class="text-red-500">{{ array_sum(array_column(array_filter($empleado['semana']), 'retraso')) }} min</span>
                                            </div>
                                            <div>
                                                <span class="text-amber-500">{{ array_sum(array_column(array_filter($empleado['semana']), 'salida_temprana')) }}
                                                    min</span>
                                            </div>
                                        </div>
                                    </td>
