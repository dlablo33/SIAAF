<x-app-layout>
        ...
        <div class="mx-auto max-w-7xl sm:px-6 lg:px-8">
            ...
            <div class="overflow-hidden bg-white shadow-sm transition-all duration-300 hover:shadow-md dark:bg-gray-700 sm:rounded-t-lg">
                    ...
                    <div class="flex items-center space-x-4">
                        ...
                        <div x-data="{ isOpen: false }" class="relative flex justify-center">
                            <button @click="isOpen = true" id="newPresacionBtn"
                                ...
                                <i class="fas fa-plus-circle mr-2"></i> Nueva Prestacion
                            </button>
                            ...
                            <div x-show="isOpen" x-transition:enter="transition duration-300 ease-out"
                                ...
                                <div
                                    ...
                                    <form method="POST" action="{{ route('rh.prestaciones.store') }}">
                                        @csrf
                                        <div class="mt-4">
                        ...
                        <span class="rounded-full bg-white px-3 py-1 text-sm text-gray-600 shadow-sm dark:bg-gray-700 dark:text-gray-300">
                            {{ count($prestaciones) }} Prestaciones
                        </span>
                    </div>
                    <div class="relative">
                        <input type="text" id="quickSearch" placeholder="Buscar prestacion..."
                ...
                <div x-data="{ isModalOpen: false, id: '', nombre: '' }" class="overflow-x-auto">
                    <table class="min-w-full divide-y divide-gray-200" id="prestacionesTable">
                        ...
                        <tbody class="divide-y divide-[#A0B4CB] bg-white dark:bg-gray-700" id="prestacionTableBody">
                                ...
                                <tr class="hover:bg-gray-2 00 transform transition-all hover:scale-[1.002] dark:hover:bg-gray-600">
                                    ...
                                    <td class="whitespace-nowrap px-6 py-4 text-sm font-medium">
                                        <div class="flex justify-center space-x-2">
                                            <button @click="isModalOpen = true; id = '{{ $prestacion->id }}'; nombre = '{{ $prestacion->nombre }}'" id="newPresacionBtn"
                                            ...
                                            <button class="rounded-full p-2 text-red-600 transition-colors hover:bg-red-50 hover:text-red-900" title="Eliminar"
                                                data-tooltip-target="tooltip-delete" onclick="confirmDelete({{ $prestacion->id }})">
                        ...
                        <div x-show="isModalOpen" x-transition:enter="transition duration-300 ease-out"
                            ...
                            <div
                                ...
                                <div class="mt-4 sm:-mx-2 sm:mt-6 sm:flex sm:items-center">
                                    ...
                                    <button type="button" onclick="editprestacion()"
                                        class="mt-3 w-full transform rounded-md bg-indigo-600 px-4 py-2 text-sm font-medium capitalize tracking-wide text-white transition-colors duration-300 hover:bg-indigo-700 focus:outline-none sm:mx-2 sm:mt-0 sm:w-1/2">
                                        Guardar
                                    </button>
                                </div>
                    ...
                    <div class="flex flex-1 items-center justify-between">
                        <div class="text-sm text-gray-700 dark:text-gray-300">
                            @if ($prestaciones->count() > 0)
                                Mostrando <span class="font-medium">{{ $prestaciones->firstItem() }}</span>
                                a
                                <span class="font-medium">{{ $prestaciones->lastItem() }}</span> de
                                <span class="font-medium">{{ $prestaciones->total() }}</span> resultados
                            @else
                                No se encontraron resultados
                            @endif
                        </div>
                        <div class="flex space-x-2">
                            @if ($prestaciones->onFirstPage())
                                <span class="cursor-not-allowed rounded-md bg-gray-100 px-3 py-1 text-gray-400 dark:bg-gray-600">
                                    Anterior
                                </span>
                            @else
                                <a href="{{ $prestaciones->previousPageUrl() }}"
                                    class="rounded-md bg-white px-3 py-1 text-gray-700 transition-colors hover:bg-gray-50 dark:bg-gray-600">
                                    Anterior
                                </a>
                            @endif

                            @foreach ($prestaciones->getUrlRange(1, $prestaciones->lastPage()) as $page => $url)
                                @if ($page == $prestaciones->currentPage())
                            ...
                            @if ($prestaciones->hasMorePages())
                                <a href="{{ $prestaciones->nextPageUrl() }}"
                                    class="rounded-md bg-white px-3 py-1 text-gray-700 transition-colors hover:bg-gray-50 dark:bg-gray-600">
                                    Siguiente
                                </a>
                            @else
    ...
    @push('scripts')
        <script>
            ...
            function applyFilters() {
                ...
                const rows = document.querySelectorAll('#prestacionTableBody tr');
    ...
    <script>
        ...
        function confirmDelete(id) {
            if (confirm('¿Estás seguro de que deseas eliminar esta prestacion?')) {
                axios.post('{{ route('rh.prestaciones.destroy', ':id') }}'.replace(':id', id), {
        ...
        function editprestacion() {
            ...
            console.log(id);
            axios.post(`/rh/prestaciones/${id}`, formData, {
                headers: {
                ...
                console.log('Error updating prestacion:', error);
            })
        }
    </script>
</x-app-layout>
