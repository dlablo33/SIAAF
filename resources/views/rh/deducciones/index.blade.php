<x-app-layout>
        ...
        <div class="mx-auto max-w-7xl sm:px-6 lg:px-8">
            ...
            <div class="overflow-hidden bg-white shadow-sm transition-all duration-300 hover:shadow-md dark:bg-gray-700 sm:rounded-t-lg">
                ...
                <div class="flex items-center justify-between border-b border-gray-200 bg-white p-4 dark:bg-gray-700">
                    <div class="flex items-center space-x-4">
                        ...
                        <div x-data="{ isOpen: false }" class="relative flex justify-center">
                            <button @click="isOpen = true" id="newDeduccionBtn"
                                ...
                                <i class="fas fa-plus-circle mr-2"></i> Nueva Deduccion
                            </button>
                            ...
                            <div x-show="isOpen" x-transition:enter="transition duration-300 ease-out"
                                ...
                                <div
                                    ...
                                    <form method="POST" action="{{ route('rh.deducciones.store') }}">
                                        @csrf
                                        <div class="mt-4">
                        ...
                        <span class="rounded-full bg-white px-3 py-1 text-sm text-gray-600 shadow-sm dark:bg-gray-700 dark:text-gray-300">
                            {{ count($deducciones) }} Deducciones
                        </span>
                    </div>
                    <div class="relative">
                        <input type="text" id="quickSearch" placeholder="Buscar deduccion..."
                ...
                <div x-data="{ isModalOpen: false, id: '', nombre: '' }" class="overflow-x-auto">
                    <table class="min-w-full divide-y divide-gray-200" id="deduccionesTable">
                        ...
                        <tbody class="divide-y divide-[#A0B4CB] bg-white dark:bg-gray-700" id="deduccionesTableBody">
                                ...
                                <tr class="hover:bg-gray-2 00 transform transition-all hover:scale-[1.002] dark:hover:bg-gray-600">
                                    ...
                                    <td class="whitespace-nowrap px-6 py-4 text-sm font-medium">
                                        <div class="flex justify-center space-x-2">
                                            <button @click="isModalOpen = true; id = '{{ $deduccion->id }}'; nombre = '{{ $deduccion->nombre }}'" id="newDeduccionBtn"
                                            ...
                                            <button class="rounded-full p-2 text-red-600 transition-colors hover:bg-red-50 hover:text-red-900" title="Eliminar"
                                                data-tooltip-target="tooltip-delete" onclick="confirmDelete({{ $deduccion->id }})">
                        ...
                        <div x-show="isModalOpen" x-transition:enter="transition duration-300 ease-out"
                            ...
                            <div
                                ...
                                <div class="mt-4 sm:-mx-2 sm:mt-6 sm:flex sm:items-center">
                                    ...
                                    <button type="button" onclick="editDeduccion()"
                                        class="mt-3 w-full transform rounded-md bg-indigo-600 px-4 py-2 text-sm font-medium capitalize tracking-wide text-white transition-colors duration-300 hover:bg-indigo-700 focus:outline-none sm:mx-2 sm:mt-0 sm:w-1/2">
                                        Guardar
                                    </button>
                                </div>
                    ...
                    <div class="flex flex-1 items-center justify-between">
                        <div class="text-sm text-gray-700 dark:text-gray-300">
                            @if ($deducciones->count() > 0)
                                Mostrando <span class="font-medium">{{ $deducciones->firstItem() }}</span>
                                a
                                <span class="font-medium">{{ $deducciones->lastItem() }}</span> de
                                <span class="font-medium">{{ $deducciones->total() }}</span> resultados
                            @else
                        ...
                        <div class="flex space-x-2">
                            ...
                            @else
                                <a href="{{ $deducciones->previousPageUrl() }}"
                                    class="rounded-md bg-white px-3 py-1 text-gray-700 transition-colors hover:bg-gray-50 dark:bg-gray-600">
                                    Anterior
                                </a>
                            @endif

                            @foreach ($deducciones->getUrlRange(1, $deducciones->lastPage()) as $page => $url)
                                @if ($page == $deducciones->currentPage())
                            ...
                            @if ($deducciones->hasMorePages())
                                <a href="{{ $deducciones->nextPageUrl() }}"
                                    class="rounded-md bg-white px-3 py-1 text-gray-700 transition-colors hover:bg-gray-50 dark:bg-gray-600">
                                    Siguiente
                                </a>
                            @else
    ...
    @push('scripts')
        <script>
            ...
            function applyFilters() {
                ...
                const rows = document.querySelectorAll('#deduccionesTableBody tr');
            ...
            document.getElementById('newDeduccionBtn').addEventListener('mouseenter', function() {
                this.classList.add('animate-bounce');
            });

            document.getElementById('newDeduccionBtn').addEventListener('mouseleave', function() {
    ...
    <script>
        ...
        function confirmDelete(id) {
            if (confirm('¿Estás seguro de que deseas eliminar esta deduccione?')) {
                axios.post('{{ route('rh.deducciones.destroy', ':id') }}'.replace(':id', id), {
                    ...
                    .catch(error => {
                        // Handle error
                        console.error('Error deleting deduccione:', error);
                    });
            }
        }

        function editDeduccion() {
            ...
            console.log(id);
            axios.post(`/rh/deducciones/${id}`, formData, {
                headers: {
                ...
                console.log('Error updating deduccione:', error);
            })
        }
    </script>
</x-app-layout>
